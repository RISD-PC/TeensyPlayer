...
void controlFileWithPot() {
  ...
  if (abs(controlPotVal - lastControlPotVal) > 3) {
    int index = map(controlPotVal, 0, 1023, 0, maxFilenameIndex - 1);
    lastControlPotVal = controlPotVal;
  }

  // playFileOnceUntilIndexChanges(int index, const char* arrayName[], int maxFilenameIndex, AudioPlaySdWav *SdWaveplayer);
